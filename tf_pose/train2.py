...
if __name__ == '__main__':
    ...
    datapath = "D:/swimcamD2/SwimData/pose/annotations_trainval2014/annotations"
    ...
    if model in ['cmu', 'vgg'] or 'mobilenet' in model:
        scale = 8
    ...
    with tf.device(tf.DeviceSpec(device_type="CPU")):
        input_node = tf.placeholder(tf.float32, shape=(batchsize, args.input_height, args.input_width, 3), name='image')
        vectmap_node = tf.placeholder(tf.float32, shape=(batchsize, output_h, output_w, 38), name='vectmap')
        heatmap_node = tf.placeholder(tf.float32, shape=(batchsize, output_h, output_w, 19), name='heatmap')

        # prepare data
        df = get_dataflow_batch(datapath, True, batchsize, img_path=imgpath)
        enqueuer = DataFlowToQueue(df, [input_node, heatmap_node, vectmap_node], queue_size=100)
        q_inp, q_heat, q_vect = enqueuer.dequeue()

    df_valid = get_dataflow_batch(datapath, False, batchsize, img_path=imgpath)
    ...
    for gpu_id in range(args.gpus):
        with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_id)):
            with tf.variable_scope(tf.get_variable_scope(), reuse=(gpu_id > 0)):
                net, pretrain_path, last_layer = get_network(model, q_inp_split[gpu_id])
                if args.checkpoint:
    ...
    with tf.device(tf.DeviceSpec(device_type="GPU")):
        ...
        total_loss_ll_paf = tf.reduce_sum(last_losses_l1) / batchsize
        total_loss_ll_heat = tf.reduce_sum(last_losses_l2) / batchsize
        total_loss_ll = tf.reduce_sum([total_loss_ll_paf, total_loss_ll_heat])

        # define optimizer
        step_per_epoch = 121745 // batchsize
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            if gs_num - last_gs_num >= 500:
                ...
                logger.info('epoch=%.2f step=%d, %0.4f examples/sec lr=%f, loss=%g, loss_ll=%g, loss_ll_paf=%g, loss_ll_heat=%g' % (gs_num / step_per_epoch, gs_num, batch_per_sec * batchsize, lr_val, train_loss, train_loss_ll, train_loss_ll_paf, train_loss_ll_heat))
                last_gs_num = gs_num
            ...
            if gs_num - last_gs_num2 >= 2000:
                ...
                outputMat = sess.run(
                    outputs,
                    feed_dict={q_inp: np.array((sample_image + val_image) * max(1, (batchsize // 16)))}
                )
